...
import { useTipoCargaMasterData } from '../../hooks/tipo-carga/useTipoCargaMasterData';
...
interface TipoCargaMasterDataPageProps {
...
export function TipoCargaMasterDataPage({ config }: TipoCargaMasterDataPageProps) {
    ...
    const handleView = (item: TipoCarga) => {
        setEditingItem(item);
        setViewMode(true);
        setFormOpen(true);
    };

    const handleEdit = (item: TipoCarga) => {
        setEditingItem(item);
        setViewMode(false);
        setFormOpen(true);
    };
    ...
    const handleDelete = async (id: number) => {
        if (window.confirm(`¿Estás seguro de que quieres eliminar este tipo de carga?`)) {
            try {
                ...
                setSnackbar({
                    open: true,
                    message: 'Error al eliminar tipo de carga',
                    severity: 'error',
                });
            }
        }
    ...
    const handleFormSubmit = async (formData: Partial<TipoCarga>) => {
        try {
            if (editingItem) {
                // Filter out fields that shouldn't be sent to the backend for updates
                const { id, tiposEmbarque, ...updateData } = formData;
                ...
                await create(formData as Omit<TipoCarga, 'id'>);
            ...
            setSnackbar({
                open: true,
                message: 'Error al guardar tipo de carga',
                severity: 'error',
            });
